<template lang="pug">
  v-form
    v-layout(row, wrap)
      v-flex(xs9)
        v-text-field(label="FotoFinder Ordner", name="folder", type="text", v-model="settings.folderPath")
      v-flex(xs3)
...
export default {
  ...
  methods: {
    save() {
      this.$electronStore.set(DB.SETTINGS_FOTOFINDER, this.settings);
    },
    selectFolder() {
  ...
  mounted() {
    this.settings = this.$electronStore.get(DB.SETTINGS_FOTOFINDER);
  },
};
</script>
