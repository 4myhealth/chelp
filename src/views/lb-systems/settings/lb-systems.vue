<template lang="pug">
  v-form
    v-layout(row, wrap)
      v-flex(xs9)
        v-text-field(label="LB-Systems Ordner", name="folder", type="text", v-model="settings.folderPath")
      v-flex(xs3)
      ...
      v-flex(xs9)
        v-text-field(label="LB-Systems Programm", name="folder", type="text", v-model="settings.programmPath")
      v-flex(xs3)
...
export default {
  ...
  mounted() {
    this.settings = this.$electronStore.get(DB.SETTINGS_LB_SYSTEMS);
  },
  methods: {
    save() {
      this.$electronStore.set(DB.SETTINGS_LB_SYSTEMS, this.settings);
    },
    selectFolder() {
